...
//	}
	...
	public page getGoodsPage(int typeID,int pageNo) {//通过分页模型来查询
		...
		p.setPageSizeAndTotalCount(9, totalCount);//通过总数 计算要分多少页
		
		List list=null;
		try {
			list=gDao.selectGoods(typeID, pageNo, 9); //查询数据 根据类型id ，页码。 返回该页的集合
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public page getGoodsRecommendPage(int type,int pageNo) {//通过分页模型来查询
		...
		List list=null;
		try {
			list=gDao.selectGoodsRecommend(type, pageNo, 9); //查询数据 根据类型id ，页码。 返回该页的集合
...
//			list=gDao.selectGoodsRecommend(type, pageNo, 8); //查询数据 根据类型id ，页码。 返回该页的集合
			list=gDao.selectSearchGoods(keyword,pageNo,9);

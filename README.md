...
(3)没有发现错误的测试也是有价值的,完整的测试是评定该网站质量的一种方法。
5.1.3	测试方法和工具
本系统采用如下方法进行测试：
（1）后台打印相关数据：在系统测试出错误的时候，可以把相关重要的数据和方法用后台打印的方法打印出来，通过打印出的数据进行分析从而找出漏洞所在。后台打印的好处是简单易行，工作量比较小。
...
6.启动tomcat。
7.打开浏览器并且输入如下网址。
http://localhost:*0*0/mail/welcome.jsp
